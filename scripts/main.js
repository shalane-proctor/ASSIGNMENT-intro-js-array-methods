...
const toggleCart = (event) => {
  if (event.target.id.includes("fav-btn")) {
   ...
   const index = referenceList.findIndex(num => num.id === Number(id))
...
const search = (event) => {
  ...
  const searchResult = referenceList.filter(userInput => 
    userInput.title.toLowerCase().includes(eventLC) ||
    userInput.author.toLowerCase().includes(eventLC) ||
    userInput.description.toLowerCase().includes(eventLC)
  )
...
// .reduce() & .some()
const cartTotal = () => {
  const cart = referenceList.filter(cartTotal => cartTotal.inCart)
